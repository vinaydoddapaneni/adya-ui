...
export class AuiTreeView extends AuiElement {
  ...
  public setNodes(nodes: AuiTreeNode[]) {
    this.nodes = nodes;
    this.render();
  }
  ...
  public expandAll() {
    const collectIds = (nodes: AuiTreeNode[]): void => {
      nodes.forEach(node => {
  ...
  private renderNode(node: AuiTreeNode): string {
  ...
  protected render() {
    ...
    this._shadowRoot.querySelectorAll('[data-toggle]').forEach(icon => {
      icon.addEventListener('click', (e) => {
        e.stopPropagation();
        const nodeId = (icon as HTMLElement).dataset.toggle || '';
    ...
    this._shadowRoot.querySelectorAll('[data-select]').forEach(content => {
      content.addEventListener('click', () => {
        const nodeId = (content as HTMLElement).dataset.select || '';
